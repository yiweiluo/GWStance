...
def get_serp_urls(l_domains,r_domains):
    ...
    for DOMAIN in l_domains + r_domains:
        for KW in CC_KEYWORDS:
            ...
            pickle.dump(results,open(os.path.join('output','serp_api','{}_{}.pkl'.format(DOMAIN,KW)),'wb'))
...
def merge_serp_urls():
    ...
    save_name = 'output/google_search_res_climate_change_{}.pkl'.format(str_curr_date)
...
def get_mc_urls(start_year=1,start_mo=1,start_day=1,end_year=2020,end_mo=12,end_day=31):
    ...
    if not os.path.exists(os.path.join('output','mediacloud',date_range_str)):
        os.mkdir(os.path.join('output','mediacloud',date_range_str))
        ...
        for curr_outlet_ix in mc_ids.index:
            ...
            if len(stories) > 0:
                ...
                df.to_pickle(os.path.join('output','mediacloud',date_range_str,'{}_df.pkl'.format(OUTLET_NAME)))
        ...
        dfs = []
        for filename in glob.glob('output/mediacloud/{}/*.pkl'.format(date_range_str)):
        ...
        shutil.rmtree(os.path.join('output','mediacloud',date_range_str))
...
def create_filtered_df(l_domains=None,r_domains=None,mc_date_range_str=None):
    ...
    if args.do_serp:
        google_pkl = glob.glob("output/google_search_res_climate_change_*")
