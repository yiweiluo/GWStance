...
from classifier.glue_metrics_plus import glue_compute_metrics as compute_metrics
#from transformers import glue_output_modes as output_modes
#from transformers import glue_processors as processors
from classifier.glue_processors_plus import glue_processors as processors
from classifier.glue_processors_plus import glue_output_modes as output_modes
from classifier.glue_weighted import glue_convert_examples_to_features as convert_examples_to_features
...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            instance_weights = batch[4].to(args.device)
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                tmp_eval_loss = loss_fct(logits.view(-1, model.num_labels), labels.view(-1))
                tmp_eval_loss = (instance_weights * tmp_eval_loss).mean()
        ...
        if args.metrics is None:
            ...
            result = compute_metrics(args.metrics, preds, out_label_ids)
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if args.processor is None:
        ...
        output_mode = output_modes[args.processor]
    # Load data features from cache or dataset file
    cached_features_file = os.path.join(
        args.data_dir,
        "cached_{}_{}_{}_{}".format(
            #"train" if not evaluate else ("test" if args.eval_on_test else "dev"),
            "train" if not evaluate else args.eval_partition,
    ...
    dataset = TensorDataset(all_input_ids, all_attention_mask, all_token_type_ids, all_labels, all_weights)
    return dataset
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help="Override the processor to use (e.g. with binary).",
    )
    parser.add_argument(
        "--metrics",
        default=None,
        type=str,
        required=False,
        help="Override the metrics to use (e.g. with binary).",
    )
    parser.add_argument(
        "--output_dir",
        default=None,
    ...
    parser.add_argument(
        "--do_text_b", action="store_true", help="Set this flag if you want to set up 2-sequence classification.",
    )
    ...
    if args.processor is None:
        ...
        if args.processor not in processors:
            raise ValueError("Processor not found: %s" % (args.processor))
        processor = processors[args.processor]()
        args.output_mode = output_modes[args.processor]
    label_list = processor.get_labels()
